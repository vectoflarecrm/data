...
<section class="panel hidden" id="detailPanel"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><h2 style="margin:0">客户详情</h2><div><button class="button" id="editBtn">编辑</button> <button class="button secondary" id="closeDetail">关闭</button></div></div><div id="detailContent"></div><div id="detailMessage" class="notice hidden"></div></section>
...
(function(){
  ...
  var statusBadge=function(s){return '<span class="badge badge-'+esc(s)+'">'+esc(s)+'</span>'};
  var load=function(){var params=new URLSearchParams({q:$('search').value,status:$('status').value,limit:String(state.limit),offset:String(state.offset)});api('/admin/api/customers?'+params.toString()).then(function(data){state.total=data.total; $('summary').textContent='共 '+data.total+' 条'; $('rows').innerHTML=data.items.map(function(c){return '<tr><td>'+esc(c.id)+'</td><td style="font-size:12px">'+esc(c.company_id)+'</td><td><a href="'+esc(c.domain)+'" target="_blank" rel="noopener">'+esc(c.domain)+'</a></td><td>'+statusBadge(c.status)+'</td><td>'+esc((c.customer_segment||'-').slice(0,60))+'</td><td>'+esc(c.updated_at||'-')+'</td><td><button class="button" onclick="window.viewCustomer('+c.id+')">查看详情</button></td></tr>'}).join('')||'<tr><td colspan="7">暂无数据</td></tr>'; $('pageInfo').textContent=(state.total?state.offset+1:0)+'-'+Math.min(state.offset+state.limit,state.total)+' / '+state.total; $('prev').disabled=state.offset===0; $('next').disabled=state.offset+state.limit>=state.total}).catch(function(error){showMessage('listMessage',error.message,false)})};
  var renderDetail=function(c){var html='<div class="detail-grid">';
    html+='<div class="field"><div class="field-label">数据库 ID</div><div class="field-value">'+esc(c.id)+'</div></div>';
    html+='<div class="field"><div class="field-label">状态</div><div class="field-value">'+statusBadge(c.status)+'</div></div>';
    html+='<div class="field"><div class="field-label">公司 ID</div><div class="field-value" style="font-size:12px;word-break:break-all">'+esc(c.company_id)+'</div></div>';
    html+='<div class="field"><div class="field-label">更新时间</div><div class="field-value">'+esc(c.updated_at||'-')+'</div></div>';
    html+='<div class="field field-full"><div class="field-label">企业网址</div><div class="field-value"><a href="'+esc(c.domain)+'" target="_blank" rel="noopener">'+esc(c.domain)+'</a></div></div>';
    html+='<div class="field field-full"><div class="field-label">客户细分 (Customer Segment)</div><div class="field-value">'+esc(c.customer_segment||'未分析')+'</div></div>';
    html+='</div>';
    var personas=c.personas_and_solutions; var parsed=null;
    if(personas){try{parsed=JSON.parse(personas)}catch(e){}}
    if(parsed){
      var personaList=parsed.personas||[]; var solutionList=parsed.solutions||[];
      if(personaList.length>0){html+='<div class="section-title">👤 客户画像 (Personas)</div>';personaList.forEach(function(p){html+='<div class="persona-card"><h4>'+esc(p.name||'未知角色')+'</h4><ul>';(p.needs||[]).forEach(function(n){html+='<li>'+esc(n)+'</li>'});html+='</ul></div>'});}
      if(solutionList.length>0){html+='<div class="section-title">💡 解决方案 (Solutions)</div>';solutionList.forEach(function(s){html+='<div class="solution-card"><h4>'+esc(s.name||'')+'</h4><p>'+esc(s.value||'')+'</p></div>'});}
      if(personaList.length===0&&solutionList.length===0){html+='<div class="section-title">📋 AI 分析结果</div><p style="color:#64748b">该客户暂无画像和解决方案数据</p>'};
    }else if(personas){html+='<div class="section-title">📋 AI 分析结果</div><pre style="background:#f8fafc;padding:12px;border-radius:8px;font-size:12px;overflow-x:auto;white-space:pre-wrap">'+esc(personas)+'</pre>'}
    html+='<div class="section-title">📝 中文备注 (Remarks)</div><div style="background:#f8fafc;padding:14px;border-radius:8px;font-size:14px;white-space:pre-wrap;line-height:1.6">'+esc(c.remarks||'暂无备注')+'</div>';
    return html};
  window.viewCustomer=function(id){api('/admin/api/customers/'+id).then(function(c){state.selected=id;$('detailPanel').classList.remove('hidden');$('editPanel').classList.add('hidden');$('detailContent').innerHTML=renderDetail(c);$('detailMessage').classList.add('hidden');window.scrollTo({top:0,behavior:'smooth'})}).catch(function(error){showMessage('listMessage',error.message,false)})};
  window.editCustomer=function(id){api('/admin/api/customers/'+id).then(function(c){state.selected=id;$('editPanel').classList.remove('hidden');$('detailPanel').classList.add('hidden');$('editId').value=c.id;$('editCompanyId').value=c.company_id;$('editDomain').value=c.domain||'';$('editStatus').value=c.status;$('editSegment').value=c.customer_segment||'';$('editPersonas').value=c.personas_and_solutions||'';$('editRemarks').value=c.remarks||'';window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'})}).catch(function(error){showMessage('listMessage',error.message,false)})};
  $('load').onclick=function(){state.offset=0;load()};$('search').onkeydown=function(event){if(event.key==='Enter'){state.offset=0;load()}};$('status').onchange=function(){state.offset=0;load()};$('prev').onclick=function(){if(state.offset>0){state.offset=Math.max(0,state.offset-state.limit);load()}};$('next').onclick=function(){if(state.offset+state.limit<state.total){state.offset+=state.limit;load()}};$('closeDetail').onclick=function(){$('detailPanel').classList.add('hidden')};$('cancel').onclick=function(){$('editPanel').classList.add('hidden')};
  $('editBtn').onclick=function(){if(state.selected!==null)window.editCustomer(state.selected)};
